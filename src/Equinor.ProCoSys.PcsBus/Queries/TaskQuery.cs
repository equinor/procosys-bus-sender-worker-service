...
public class TaskQuery
{
    public static string GetQuery(long? taskId, string plant= null)
    {
        ...
        // TaskParentProCoSysGuid
        var sql = @$"select
       '{{""Plant"": ""' || ec.ProjectSchema ||
       '"", ""ProCoSysGuid"" : ""' || ec.ProCoSys_Guid ||
       '"", ""TaskParentProCoSysGuid"" : ""' || ec2.ProCoSys_Guid ||
       '"", ""PlantName"" : ""' || ps.Title ||
       '"", ""ProjectName"" : ""' || (SELECT project.Title FROM Document LEFT JOIN Project ON Document.Project_Id=Project.Project_id WHERE Document.DOCUMENT_ID = (SELECT   MIN (e.element_id) AS rootElement_Id FROM ELEMENTCONTENT e START WITH e.ELEMENT_ID = ec.Element_Id CONNECT BY PRIOR e.parent_Id = e.element_id GROUP BY e.parent_id HAVING parent_id IS NULL)) ||
